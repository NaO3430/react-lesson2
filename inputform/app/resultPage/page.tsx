...
function resultPage() {
  ...
  const convertGenderToJapanese = (gender: string | null): string => {
    switch (gender) {
      case 'men':
        return '男性';
      case 'women':
  ...
  const convertAnswerToJapanese = (answer: string | null): string => {
    switch (answer) {
      case 'yes':
        return 'はい';
      case 'no':
  ...
  return (
    <div className="flex flex-col justify-center h-screen mx-auto max-w-xl space-y-4">
      ...
      <h2>-性別-</h2>
      <p>{convertGenderToJapanese(gender)}</p>
      <h2>-生年月日-</h2>
      <p>
        {year}年{month}月{day}日
      </p>
      <h2>-現在、生命保険に加入されていますか？-</h2>
      <p>{convertAnswerToJapanese(answer1)}</p>
      <h2>
        -現在入院中ですか。または、最近3ヶ月以内に医師の診察・検査の結果、入院・手術をすすめられたことはありますか？-
      </h2>
      <p>{convertAnswerToJapanese(answer2)}</p>
      <h2>
        -過去5年以内に、病気やけがで、手術を受けたことまたは継続して7日以上の入院をしたことがありますか？-
      </h2>
      <p>{convertAnswerToJapanese(answer3)}</p>
      <h2>-ご相談内容-</h2>
